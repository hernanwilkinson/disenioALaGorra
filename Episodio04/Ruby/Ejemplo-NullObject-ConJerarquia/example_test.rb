...
class ExampleTest < MiniTest::Unit::TestCase
  ...
  def test_can_not_create_address_with_empty_street
    assert_raises_with_message(Exception,
                               ProvidedAddress::INVALID_STREET) { ProvidedAddress.at "", olivos, olivos_zip_code }
  end

  def test_can_not_create_address_with_empty_city
    assert_raises_with_message(Exception,
                               ProvidedAddress::INVALID_CITY) { ProvidedAddress.at 'Maipu 111', '', olivos_zip_code }
  end
  ...
  def address_at_olivos
    ProvidedAddress.at 'Maipu 111', olivos, olivos_zip_code
  end

  def address_not_provided
    NotProvidedAddress.new
  end
end
